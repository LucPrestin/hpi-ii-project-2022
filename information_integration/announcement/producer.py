...
class AnnouncementProducer:
    ...
    def produce_to_topic(self, announcement: Announcement) -> None:
        if announcement.id not in self.used_ids:
            self.used_ids.append(announcement.id)

            self.producer.produce(
                topic=TOPIC,
                partition=-1,
                key=str(announcement.id),
                value=announcement,
            ...
            log.info(f'Announcement {announcement.id} already produced to {TOPIC}, skipping entry')
